...
if (!function_exists('paypal_payment')) {
    ...
    function paypal_payment($amt, $invoice_id){
		...
		$invoice  = $invoice_id;
		$demo = 1;
		$data  = array('invoice_id'=>$invoice, 'amount'=>$amount);
...
if (!function_exists('_allProducts')) {
    ...
    function _allProducts(){
		...
		$res   = $query->result();
		if(!empty($res)){ return $res; }else{ return array(); }
    }
}	
...
if (!function_exists('_orderDetails')) {
    ...
    function _orderDetails($id){
		...
		if(!empty($result)){ return array_flatten($result); }else{ return array(); }
    }
}		
...
if (!function_exists('_wURL')) {
    ...
    function _wURL(){
		...
		if(!empty($res)){ return $res->option_value; }else{ return array(); }
    }
}
...
if (!function_exists('_unitPrice')) {
    ...
    function _unitPrice($pid){
		...
		if(!empty($res)){ return $res->min_price; }else{ return array(); }
    }
}
...
if (!function_exists('_thumbnail')) {
    ...
    function _thumbnail($id){
		...
		if(!empty($res)){ 
			...
			$thumb = _wURL().'/wp-content/uploads/'.$img; return $thumb;
		}else{ 
			$str = ''; return $str; 
		}
    }
