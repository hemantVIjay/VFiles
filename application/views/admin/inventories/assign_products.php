            ...
            <div class="tile-content" id="tile-content">
				...
				<table class="table table-bordered" id="orderss">
				 <thead>
				  ...
				  <th width="13%">Recommended Extra <br />(For Pending Orders)</th>
				  <th width="14%">Provide Quantity(KG)</th>
				 ...
				 <?php if (!empty($purchases)): $i = 1; foreach ($purchases as $purchase): ?>
				     <tr>
					 <td><?= $i; ?></td>
					 <td><input type="hidden" class="form-control" name="product_id" value="<?= $purchase->product_id; ?>" >
					 <?php $pr = _productInfo($purchase->product_id); echo$pr->post_title; ?></td>
					 <td><input type="hidden" class="form-control nUMBER" name="purchase_id" value="<?= $purchase->purchase_id; ?>" readonly data-available="<?= _availableQuantity($purchase->purchase_id); ?>"><?= _availableQuantity($purchase->purchase_id); ?></td>
					 <td><input type="number" class="form-control nUMBER" name="recommended_quantity" value="10"></td>
					 <td><input type="number" class="form-control nUMBER" name="assigned_quantity" placeholder="Enter Quantity"></td>
					 ...
					 <td><button name="update_quantity" id="update_quantity" class="btn btn-success" onclick="submit_purchase(this, '<?php echo$purchase->R_ID; ?>')">Assign</button></td>
					 </tr>
				   <?php $i++; endforeach;
    endif;
?>
				 ...
				 <?php if (!empty($d_users)): $i = 1; foreach ($d_users as $u_row): ?>
					 ...
					 <?php foreach($products as $rows){  ?>
				        <td><?php $data['user_id'] = $u_row->id;
						          ...
						          $sdata = $this->inventory_model->get_assignMent($data); 
								  if(!empty($sdata)){
									echo $sdata['assigned_quantity'];   
								  }else{
									echo'0';
								  }								  
						    ?>
...
function submit_purchase(e, r_id){
	...
	//alert(r_id)
	//debugger
        var available = $(e).closest("tr").find('td:eq(2)').find('input').attr('data-available');
	
        var product_id = $(e).closest("tr").find('td:eq(1)').find('input').val();
        var purchase_id = $(e).closest("tr").find('td:eq(2)').find('input').val();
		var rec_quantity = $(e).closest("tr").find('td:eq(3)').find('input').val();
        var assigned_quantity = $(e).closest("tr").find('td:eq(4)').find('input').val();
        var user_id = $(e).closest("tr").find('td:eq(5)').find('select').val();
        var comments = $(e).closest("tr").find('td:eq(6)').find('input').val();
		
		var total_ass_quantity = parseInt(assigned_quantity)+parseInt(rec_quantity);
		
		if(assigned_quantity == ''){
			showAlert('warning','Invalid Quantity', 'Please add some quantity to assign.');
			return false;
		}if(parseInt(total_ass_quantity) > parseInt(available)){
		...
		//var total_ass_quantity = parseInt(assigned_quantity)+parseInt(rec_quantity);
   		$.ajax({
   		  ...
   		  type : "POST",
   		  data: {product_id:product_id, purchase_id:purchase_id, assigned_quantity:total_ass_quantity, user_id:user_id, comments:comments},
